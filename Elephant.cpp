...
void Elephant::SetCoordsShapes(int _coord[2][32], int _index)
{
	SetBoolAndColor();
	for(int i=0;i<8;i++)
	{
		for(int j=0;j<8;j++)
		{
			if((abs(_coord[0][_index]-i)==abs(_coord[1][_index]-j))&&((i!=_coord[0][_index])||(j!=_coord[1][_index])))
			{
				SetBool(i,j,true);
			}
...
void Elephant::DrawShapes(RenderWindow& window)
{
	Texture tShape;
